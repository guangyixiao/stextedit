...
namespace xpm {
	...
	XpmSchema::XpmSchema(QWidget *parent)
		: QMainWindow(parent),
		...
		textEdit(0)
	{
		...
		//XpmSchema = new QXpmSchema(this);
		//we need the xpm control right now and make use the loaded vocabulary
		//xpmControl = new XpmTermDocControl(&_termControl);
		...
		/*if (!load(initialFile))
			fileNew();*/
		QString initialFile("data/example_en.sebds");
	...
	void XpmSchema::closeEvent(QCloseEvent *e)
	{
		if (maybeSave())
			e->accept();
		else
	...
	void XpmSchema::setupFileActions()
	{
	...
	void XpmSchema::setupEditActions()
	{
		QToolBar *tb = new QToolBar(this);
	...
	void XpmSchema::setupTextActions()
	{
		QToolBar *tb = new QToolBar(this);
	...
	void XpmSchema::createMapDockWindow() {
	...
	void XpmSchema::setupViewActions() {
	...
	void XpmSchema::setupTableActions()
	{
		QToolBar *tb = new QToolBar(this);
	...
	void XpmSchema::setupSimuActions()
	{
		QToolBar *tb = new QToolBar(this);
	...
	bool XpmSchema::load(const QString &f)
	{
		if (!QFile::exists(f))
			return false;
		QFile file(f);
	...
	bool XpmSchema::loadInitTable(string lang) {
		QString initialFile = lang == "en" ? "data/xpms_en.sebds" : "data/xpms_chs.sebds";		
		bool result = loadTable(initialFile);
		setCurrentFileName(tr(""));
		return result;
	}

	bool XpmSchema::loadTable(const QString &f)
	{
		if (!QFile::exists(f))
			return false;
		QFile file(f);
	...
	//void XpmSchema::loadVoc(QString filename) {
	...
	void XpmSchema::loadSemantic(const QString &f) {
		//f.sp
	...
	void XpmSchema::setCurrentFileName(const QString &fileName)
	{
	...
	void XpmSchema::fileNewChs()
	{
		if (maybeSave() && !loadInitTable("chs")) {
			...
			rows = 100;
			//QTextCursor cursor(textEdit->textCursor());
			cursor.insertText("\r\nThis is the content of the instance document:");
	...
	void XpmSchema::fileOpenTable()
	{
		QString fn = QFileDialog::getOpenFileName(this, tr("Open File..."),
			QString(), tr("SEDBS_Files(*.sebds);;HTML_Files (*.htm *.html);;Json-Files (*.json);;Text-Files (*.txt);;All Files (*)"));
		if (!fn.isEmpty())
			loadTable(fn);
	}

	bool XpmSchema::saveSemantic() {
	...
	bool XpmSchema::fileSaveAsTable()
	{
		QString fn = QFileDialog::getSaveFileName(this, tr("Save as..."),
			QString(), tr("SEDBS-Files(*.sebds);;HTML-Files(*.htm *.html);;Json files (*.json);;Text-Files (*.txt);;All Files (*)"));
		if (fn.isEmpty())
			return false;
		if (! (fn.endsWith(".sebds", Qt::CaseInsensitive)) || (fn.endsWith(".html", Qt::CaseInsensitive) || fn.endsWith(".htm", Qt::CaseInsensitive) || fn.endsWith(".json", Qt::CaseInsensitive) || fn.endsWith(".txt", Qt::CaseInsensitive)) )
			fn += ".html"; // default
		setCurrentFileName(fn);
		return fileSaveTable();
	}

	void XpmSchema::filePrint()
	{
#ifndef QT_NO_PRINTER
	...
	void XpmSchema::filePrintPreview()
	{
#ifndef QT_NO_PRINTER
		QPrinter printer(QPrinter::HighResolution);
	...
	void XpmSchema::printPreview(QPrinter *printer)
	{
#ifdef QT_NO_PRINTER
		Q_UNUSED(printer);
#else
	...
	void XpmSchema::textFamily(const QString &f)
	{
		QTextCharFormat fmt;
		fmt.setFontFamily(f);
		mergeFormatOnWordOrSelection(fmt);
	...
	void XpmSchema::textSize(const QString &p)
	{
	...
	void XpmSchema::textStyle(int styleIndex)
	{
		QTextCursor cursor = textEdit->textCursor();
	...
	void XpmSchema::textAlign(QAction *a)
	{
	...
	void XpmSchema::currentCharFormatChanged(const QTextCharFormat &format)
	{
		fontChanged(format.font());
		colorChanged(format.foreground().color());
	}

	void XpmSchema::cursorPositionChanged()
	{
		alignmentChanged(textEdit->alignment());
	}

	void XpmSchema::clipboardDataChanged()
	{
#ifndef QT_NO_CLIPBOARD
		actionPaste->setEnabled(!QApplication::clipboard()->text().isEmpty());
#endif
	...
	void XpmSchema::mergeFormatOnWordOrSelection(const QTextCharFormat &format)
	{
		QTextCursor cursor = textEdit->textCursor();
	...
	void XpmSchema::fontChanged(const QFont &f)
	{
	...
	void XpmSchema::colorChanged(const QColor &c)
	{
		QPixmap pix(16, 16);
		pix.fill(c);
	...
	void XpmSchema::alignmentChanged(Qt::Alignment a)
	{
		if (a & Qt::AlignLeft) {
	...
	void XpmSchema::selectionChanged() {
		QTextCursor cursor(textEdit->textCursor());
		...
		if(selectedtext.count() > 0) {			
			...
			selectionrange.word_ix = cursor.selectionStart();
	...
	void XpmSchema::highlightSelection(int pos, int length) {
		QTextCursor cursor(textEdit->textCursor());
	...
	void XpmSchema::disHighlightSelection(int pos, int length) {
		QTextCursor cursor(textEdit->textCursor());
		cursor.setPosition(pos);
		cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, length);
	...
	void XpmSchema::selectMapRange(int pos, int length) {
		QTextCursor cursor = textEdit->textCursor();
		cursor.setPosition(pos);
		cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, length);
		textEdit->setTextCursor(cursor);
	...
	void XpmSchema::clearMapsFormats() {
		for( sfa_blocks::iterator it = xpmControl->model()->blocks().begin(); it != xpmControl->model()->blocks().end(); ++it)  {
			for( sfa_maps::iterator tit = (*it)->maps().begin(); tit != (*it)->maps().end(); ++tit) {
	...
	void XpmSchema::addCharsInMiddle() {
	...
	void XpmSchema::removeCharsInMiddle() {
		QTextCursor cursor(textEdit->textCursor());		
		QTime tm;
		int total = 0;
		for( int i = 0; i < 10; ++i) {
	...
	void XpmSchema::translate() {
		if(maybeSave()) {
			...
			XpmTableDocControl outControl(&_termFactory);
			...
			out_filename += outControl.lang() + ".sebds";
